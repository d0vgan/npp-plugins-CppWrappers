...
const TCHAR* const ERR_HOSTED_DLL_NOT_LOADED = _T("Failed to load " HOSTED_PLUGIN_DLL_NAME " !\n") \
                                               ...
                                               _T("https://sourceforge.net/projects/nppmenusearch/files/");
...
{
    InvokeHostedPluginFunction(N_MENU_SEARCH);
}

void clear_recently_used_func()
{
    InvokeHostedPluginFunction(N_CLEAR_RECENTLY_USED);
}

void repeat_prev_func()
{
    InvokeHostedPluginFunction(N_REPEAT_PREV);
}
...
void InitializePluginMenu()
{
    ...
    // init shortcuts:
    InitShortcut(N_MENU_SEARCH, false, true, false, VK_F1); // Ctrl+F1

    // init menu items:
    InitFuncItem(N_MENU_SEARCH,         _T("Menu Search..."),               menu_search_func,         &g_funcShortcut[N_MENU_SEARCH]);
    InitFuncItem(N_CLEAR_RECENTLY_USED, _T("Clear \"Recently Used\" List"), clear_recently_used_func, &g_funcShortcut[N_CLEAR_RECENTLY_USED]);
    InitFuncItem(N_REPEAT_PREV,         _T("Repeat previous command"),      repeat_prev_func,         &g_funcShortcut[N_REPEAT_PREV]);
